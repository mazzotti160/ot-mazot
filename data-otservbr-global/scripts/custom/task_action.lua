function endTaskModalWindow(player, storage)
    ...
    -- Verificação se 'data' e 'data.total' estão definidos
    if not data or not data.total then
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Error: Task data is not available.")
        return
    end
    ...
    -- Verificação se 'data.rewards' está definido antes de usá-lo
        ...
        if player:getStorageValue(taskOptions.bonusReward) >= 1 then
            ...
            for _, info in pairs(data.rewards) do
                ...
                elseif tonumber(info[1]) then
                    ...
                    player:setStorageValue(65002, -1)
...
function confirmTaskModalWindow(player, storage)
    ...
    -- Verificação se 'data' está definido
    if not data then
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Error: Task data is not available.")
        return
    end
