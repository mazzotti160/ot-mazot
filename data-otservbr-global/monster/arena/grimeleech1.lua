for i = 1, 100 do
    local monsterTypeName = "Grimeleech" .. i
    ...
    monster.experience = math.floor(7216 * (1.2 ^ (i - 1)))  -- Aumenta a experiência em 20% a cada nível
    monster.outfit = {
        lookType = 855,
        lookHead = 0,
        lookBody = 0,
    ...
    monster.attacks = {
		{ name = "melee", interval = 2000, chance = 100, minDamage = 200 * (1.1 ^ (i - 1)), maxDamage = -435 * (1.1 ^ (i - 1)), condition = { type = CONDITION_FIRE, totalDamage = 6, interval = 9000 } },
        { name = "combat", interval = 2000, chance = 14, type = COMBAT_LIFEDRAINDAMAGE, minDamage = 100 * (1.1 ^ (i - 1)), maxDamage = -565 * (1.1 ^ (i - 1)), range = 7, shootEffect = CONST_ANI_DEATH, effect = CONST_ME_MORTAREA, target = true },
        { name = "combat", interval = 2000, chance = 12, type = COMBAT_LIFEDRAINDAMAGE, minDamage = -150 * (1.1 ^ (i - 1)), maxDamage = -220 * (1.1 ^ (i - 1)), length = 8, spread = 0, effect = CONST_ME_MAGIC_GREEN, target = false },
        { name = "combat", interval = 2000, chance = 13, type = COMBAT_DEATHDAMAGE, minDamage = -225 * (1.1 ^ (i - 1)), maxDamage = -375 * (1.1 ^ (i - 1)), radius = 4, effect = CONST_ME_MORTAREA, target = false },
        { name = "combat", interval = 2000, chance = 9, type = COMBAT_MANADRAINDAMAGE, minDamage = 0, maxDamage = -300 * (1.1 ^ (i - 1)), length = 8, spread = 3, effect = CONST_ME_MAGIC_RED, target = false },
    }
